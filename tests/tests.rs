...
fn test_get_ipv4_header_method() {
    ...
    p.add_header(headers::EthernetHeader::new(
        [6, 5, 4, 3, 2, 1],
        [1, 2, 3, 4, 5, 6],
        ethertype_numbers::ETHERTYPE_IPV4,
    ));
    p.add_header(headers::IpHeader::new(
        [192, 168, 1, 128],
        [192, 168, 1, 38],
        Protocol::TCP,
    ));
    p.add_header(headers::TcpHeader::new(3838, 3838));
    
    let tcp_header_slice = p
        .get_header_as_slice(Protocol::TCP)
        .expect("could not find tcp header");
    let bytes = 3838_u16.split_to_bytes();
    assert_eq!(bytes[0], tcp_header_slice[0]);
    assert_eq!(bytes[1], tcp_header_slice[1]);
    
    let eth_header_slice = p
        .get_header_as_slice(Protocol::ETH)
        .expect("could not find eth header");
...
fn test_get_ipv6_header_method() {
    ...
    p.add_header(headers::TcpHeader::new(59002, 80));
    
    let tcp_header_slice = p
        .get_header_as_slice(Protocol::TCP)
        .expect("could not find tcp header");
    println!("\n## TCP Header\n{:?}", tcp_header_slice);
    let bytes = 59002_u16.split_to_bytes();
    assert_eq!(bytes[0], tcp_header_slice[0]);
    assert_eq!(bytes[1], tcp_header_slice[1]);
...
fn test_eth_parse_function() {
    ...
    assert_eq!(
        eth_header_struct.get_eth_type(),
        &ethertype_numbers::ETHERTYPE_IPV4
    );
}
